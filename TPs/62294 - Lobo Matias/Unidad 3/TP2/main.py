...
def validar_estado(estado: str):
    ...
    if estado not in estados_validos:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail={"error": "Estado inválido. Debe ser 'pendiente', 'en_progreso' o 'completada'"})
...
def create_tarea(tarea: TareaCreate):
    ...
    if not tarea.descripcion.strip():  
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail={"error": "La descripción no puede estar vacía"})
...
def completar_todas():
    print(f"Endpoint PUT /tareas/completar_todas ejecutado")
    print(f"tareas antes: {tareas_db}")
    for tarea in tareas_db:
        tarea.estado = Estado.COMPLETADA
        print(f"tarea despues: {tareas_db}")
